...
WIRING_PI_DIR="/opt/rpi/sysroot/usr/local/lib"

if [[ ! -e "$WIRING_PI_DIR" ]]; then
    echo "Required path does not exist: $WIRING_PI_DIR" >&2
    exit 3
fi
...
# copy WiringPi build
echo "Copying WiringPi build in $WIRING_PI_DIR to package root"
mkdir -p "${PACKAGE_ROOT}/usr/local/lib"
cp -d "$WIRING_PI_DIR"/libwiringPi.* "${PACKAGE_ROOT}/usr/local/lib"
