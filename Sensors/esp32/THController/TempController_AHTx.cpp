...
void TempControllerAHTx::update() {
    ...
    if (!aht.begin()) {
      ...
      taskManager->getTask<MQTTController*>(MQTT_CONTROLLER)->publishWarn(F("Cannot find AHT"));
    } else {
      isInitialized = true;
    }
  } else {
    aht.getEvent(&humidity, &temp);// populate temp and humidity objects with fresh data
    if (temp.temperature > TEMP_MIN && temp.temperature < TEMP_MAX) {
      ...
      taskManager->getTask<MQTTController*>(MQTT_CONTROLLER)->publishWarn(F("Invalid temperature value"));
    }
