...
ScriptBase::ScriptBase(QString name, DatamodelBase* datamodel, LocalStorage* localStorage, TaskStorage *taskStorage, ValueManagerBase* valueManager, ActorManager *actorManager, QObject *parent) : QObject(parent), Identifyable(name)
  , m_datamodel(datamodel), m_localStorage(localStorage), m_taskStorage(taskStorage), m_valueManager(valueManager), m_actorManager(actorManager)
{
